...
1. `/slz-discover` — produces `findings.json` + `discover.summary.{md,json}`. Then call `ask_user` with a boolean `proceed` field titled **"Discover complete — continue to Reconcile?"**. The form's `message` field MUST contain: (a) a bounded excerpt of `discover.summary.md` (header, per-module status table, top observations — under ~40 lines), and (b) the path `artifacts/<run>/discover.summary.md`.
2. `/slz-reconcile` — produces `mg_alias.json` + `reconcile.summary.{md,json}`. Greenfield runs short-circuit to an all-null alias with a single boolean `ask_user`; brownfield runs prompt per-role mapping decisions via `ask_user` enums. Then call `ask_user` with a boolean `proceed` field titled **"Reconcile complete — continue to Evaluate?"**. The `message` MUST contain: (a) an excerpt of `reconcile.summary.md` (header, mode, roles-mapped count, mapping table — under ~25 lines), and (b) the path `artifacts/<run>/reconcile.summary.md`.
3. `/slz-evaluate` — produces `gaps.json` + `evaluate.summary.{md,json}`. Then call `ask_user` with a boolean `proceed` field titled **"Evaluate complete — continue to Plan?"**. The `message` MUST contain: (a) excerpt of `evaluate.summary.md` (header, severity tally, compliance ratio, top 5 gaps — under ~30 lines), and (b) the path `artifacts/<run>/evaluate.summary.md`.
4. `/slz-plan` — produces `plan.md` / `plan.json` / `plan.summary.{md,json}`. Then call `ask_user` with a boolean `proceed` field titled **"Plan reviewed — continue to Scaffold?"**. The `message` MUST contain: (a) excerpt of `plan.summary.md` (readiness snapshot, order-of-operations counts — under ~30 lines), and (b) the paths `artifacts/<run>/plan.summary.md` and `artifacts/<run>/plan.md`.
5. `/slz-scaffold` — produces Bicep + params + `scaffold.summary.{md,json}`. Then call `ask_user` with a boolean `approved` field titled **"Scaffold complete — run `az deployment mg what-if` yourself before acting. Acknowledged?"**. The `message` MUST contain: (a) short excerpt of `scaffold.summary.md` (emitted-template count, warnings count, gaps-not-scaffolded count — under ~20 lines), (b) the path `artifacts/<run>/scaffold.summary.md` (which holds the exact `az deployment` commands), and (c) the explicit reminder that the plugin never runs `create` — the operator does, after `what-if`.
